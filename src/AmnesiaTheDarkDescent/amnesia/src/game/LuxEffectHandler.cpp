...
void cLuxEffect_EmotionFlash::Start(const tString &asTextCat, const tString &asTextEntry, const tString &asSound)
{
	...
	if (asTextCat == "StreamnesiaLiteral")
	{
		std::wstring wLiteralHint(asTextEntry.length(), L' ');
		std::copy(asTextEntry.begin(), asTextEntry.end(), wLiteralHint.begin());

		sText = wLiteralHint;
	}
	else
	{
		sText = kTranslate(asTextCat, asTextEntry);
	}
	mvTextRows.clear();
